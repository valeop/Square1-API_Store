...
3. Let's test the endpoints showed below:
   ...
   - **POST | Create a new order:** `http://127.0.0.1:8000/api/v1/orders/create` This endpoint needs the token provided (Used in Bearer Token Auth method on Postman).
     - **JSON body**
       ...
       - If there are not items in shopping cart, it will return a JSON report and does not create any order.
       - In the body is not necessary to create any OrderItem key because every data is automatically extracted from CartItem and added as an OrderItem that belongs to the order that will be created.
